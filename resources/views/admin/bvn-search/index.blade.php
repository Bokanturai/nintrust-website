...
@section('title', 'Admin - BVN Search Requests')

@section('content')
<div class="page-title mb-4">
    <div class="row align-items-center">
        <div class="col-sm-6">
            <h3 class="fw-bold text-primary">BVN Search Management</h3>
            <p class="text-muted small mb-0">Oversee and synchronize BVN Phone Search requests.</p>
        </div>
    </div>
</div>
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-header bg-primary text-white py-3 d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 fw-bold text-white"><i class="mdi mdi-format-list-bulleted me-2"></i>All Search Requests</h5>
                ...
                <div class="card-body p-4">
                    ...
                    <form method="GET" class="row g-3 mb-4 bg-light p-3 rounded-3 border">
                        <div class="col-md-5">
                            <input class="form-control border-0 shadow-sm" name="search" type="text" placeholder="Search by Phone, Ref, Ticket, Name or Email..." value="{{ request('search') }}">
                        </div>
                        <div class="col-md-3">
                            <select class="form-select border-0 shadow-sm" name="status">
                                <option value="">All Statuses</option>
                                @foreach(['pending','processing','successful','query','resolved','rejected','failed'] as $status)
                        ...
                        <div class="col-md-2">
                            <a href="{{ route('admin.bvn-search.index') }}" class="btn btn-outline-secondary w-100 shadow-sm">Reset</a>
                        </div>
                    </form>
                            ...
                            <tbody>
                                @forelse ($submissions as $submission)
                                    <tr data-row-id="{{ $submission->id }}">
                                        ...
                                        <td>
                                            <code class="text-primary fw-bold">{{ $submission->reference }}</code>
                                            @if($submission->ticket_id)
                                                <div class="small text-muted mt-1">ID: {{ $submission->ticket_id }}</div>
                                            @endif
                                        </td>
                                            ...
                                            <div class="modal fade" id="updateModal{{ $submission->id }}" tabindex="-1" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content border-0 shadow rounded-4 text-dark">
                                                        ...
                                                        <form action="{{ route('admin.bvn-search.update', $submission->id) }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content border-0 shadow rounded-4 text-dark">
                                                        <div class="modal-header bg-primary text-white">
                                                            <h5 class="modal-title fw-bold text-white">Search Details</h5>
                                                        ...
                                                        <div class="modal-body p-4 text-dark">
                                                            ...
                                                            <div class="mb-3" style="max-width: 100%;">
                                                                ...
                                                                <div class="bg-light p-3 rounded-3 border text-dark" 
                                                                     style="min-height: 60px; width: 100%; word-wrap: break-word; overflow-wrap: break-word; word-break: break-word; white-space: normal;">
                                                                    {{ $submission->comment ?? 'No response message yet.' }}
                                                                </div>
                                                            </div>

                                                            <div class="p-3 bg-light rounded-3 border">
                                                                <div class="small fw-bold text-muted mb-2 text-dark">Request Info:</div>
                                                                ...
                                                                <div class="d-flex justify-content-between text-break" style="overflow-wrap: break-word;">
                                                                    <span>Ticket/Batch:</span> <span>{{ $submission->ticket_id ?? 'N/A' }} / {{ $submission->batch_id ?? 'N/A' }}</span>
                                                                </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center py-5 text-muted">No search requests found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
...
$(document).ready(function() {
    ...
    $('.btn-check-status').on('click', function() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                let msg = 'Failed to check status.';
    ...
    $('#syncAllPending').on('click', function() {
        ...
        function processSequentially(index) {
            ...
            $.ajax({
                url: url,
                method: 'GET',
                success: function() { processSequentially(index + 1); },
                error: function() { processSequentially(index + 1); }
            });
        }
    });
});
