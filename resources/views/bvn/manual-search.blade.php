...
@section('title', 'Manual BVN Search')

@section('content')
<div class="page-title mb-4">
    <div class="row align-items-center">
        <div class="col-sm-6">
            <h3 class="fw-bold text-primary">Manual BVN Search</h3>
            <p class="text-muted small mb-0">Submit a phone number or BVN for manual investigation by an administrator.</p>
        </div>
    </div>
</div>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
                <div class="card-header bg-primary text-white py-3">
                    <h5 class="mb-0 fw-bold text-white"><i class="bi bi-search me-2"></i>New Manual Request</h5>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('user.manual-bvn-search.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="phone_number" class="form-control border-start-0 ps-0 @error('phone_number') is-invalid @enderror" 
                                       placeholder="e.g. 08012345678 or 22233344455" value="{{ old('phone_number') }}" required maxlength="11">
                                @error('phone_number')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <button type="submit" class="btn btn-primary w-100 py-3 fw-bold rounded-3 shadow-sm transition-all hover-lift">
                            <i class="bi bi-send-fill me-2"></i> Submit Manual Request
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
                    <h5 class="mb-0 fw-bold text-dark"><i class="bi bi-clock-history me-2 text-primary"></i>Recent Manual Requests</h5>
                    <form action="{{ route('user.manual-bvn-search') }}" method="GET" class="d-flex">
                                            ...
                                            <div class="d-flex gap-2 justify-content-end">
                                                ...
                                                @if(in_array($submission->status, ['pending', 'processing', 'query']))
                                                    <button type="button" 
                                                            ...
                                                            data-url="{{ route('user.manual-bvn-search.check', $submission->id) }}"
                                                            title="Check Status">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center text-muted py-5">
                                            ...
                                            No manual search requests found.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
        ...
        <div class="modal-content border-0 shadow rounded-4">
            ...
            <div class="modal-body p-4">
                <div class="mb-4" style="max-width: 100%;">
                    <label class="form-label fw-bold text-muted small text-uppercase">Admin Comment / Response</label>
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.status === 'success') {
                    ...
                    Swal.fire('Processing', response.message, 'info');
